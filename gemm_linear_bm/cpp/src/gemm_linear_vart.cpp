...
int main(int argc, char* argv[]) {
    ...
    const auto kernel_name = std::string("subgraph_GEMMModel__GEMMModel_Conv2d_linear__ret");
    ...
    std::cout << "=========== Input shape: [" << first_dim << ", " << second_dim << "] ===========" << std::endl;

    for (int b = 0; b < first_dim; ++b) {
        uint64_t data_in = 0u;
        size_t size_in = 0u;
        std::tie(data_in, size_in) = input_tensor_buffer->data({b, 0});
