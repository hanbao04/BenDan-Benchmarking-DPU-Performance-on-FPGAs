...
class VAI_Quantizer:
    def __init__(self, model_class, weight_path, input_size, batch_size):
        self.model_class = model_class
        self.weight_path = weight_path
        self.input_size = input_size
        self.batch_size = batch_size
        self.model = self.model_class(input_size, batch_size)
    ...
    def quantizer_calib(self):
        ...
        input_data = torch.randn([1, self.input_size]).to(device)
        ...
        random_input = torch.randn([1, self.input_size]).to(device)
    ...
    def quantizer_export(self, name, onnx_export=False):
        input = torch.randn([1, self.input_size])
...
for i, (name, (input, class_size)) in enumerate(configs.items()):
        ...
        quant = VAI_Quantizer(model_class, weight_path, input, class_size)
